...
export function LODEvaluate(tile: Tile, minLevel: number, maxLevel: number, threshold: number): LODAction {
	...
	if (
		...
		tile.inFrustum &&
		tile.z < maxLevel &&
		distRatio < effectiveThreshold &&
		(tile.showing || tile.z <= minLevel)
	) {
		return LODAction.create;
	}
	// Hysteresis: require 20% more distance to remove than to create,
	// preventing tile oscillation at the boost boundary during camera rotation.
	if (!tile.isLeaf && tile.z >= minLevel && distRatio > effectiveThreshold * 1.2) {
		return LODAction.remove;
	}
