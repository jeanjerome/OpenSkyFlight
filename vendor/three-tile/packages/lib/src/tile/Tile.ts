...
/** Point on the ground (y≈0) where the camera forward ray intersects — view center */
...
export class Tile extends Object3D<TTileEventMap> {
	...
	public get viewCenterFactor(): number {
		const dx = this._checkPoint.x - viewCenterGround.x;
	...
	public update(params: TileUpdateParames) {
		...
		if (this.z === 0) {
			...
			// Compute ground intersection of camera forward ray for view-center LOD
			const fwd = camera.getWorldDirection(tempVec3);
			if (fwd.y < -0.01) {
				const t = -cameraWorldPosition.y / fwd.y;
				viewCenterGround.copy(fwd).multiplyScalar(t).add(cameraWorldPosition);
			} else {
				// Looking up/horizontal — place center very far ahead
				viewCenterGround.copy(fwd).multiplyScalar(50000).add(cameraWorldPosition);
				viewCenterGround.y = 0;
			}
